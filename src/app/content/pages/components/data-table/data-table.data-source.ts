...
export class DataTableDataSource {
    ...
    loadItems(queryParams: QueryParamsModel) {
		...
		this.dataTableService.getAllItems(queryParams).pipe(
			tap(res => {
				//const result = this.baseFilter(res.entidad.lista, queryParams);result.items
				this.entitySubject.next(res.entidad.lista);
				this.paginatorTotalSubject.next(res.entidad.total);
	...
	loadItemsByDate(queryParams: QueryParamsModel, model: any) {
		this.loadingSubject.next(true);
		this.dataTableService.getAllItemsByfecha(queryParams,model).pipe(
			tap(res => {
				this.entitySubject.next(res.entidad.lista);
				this.paginatorTotalSubject.next(res.entidad.total);
